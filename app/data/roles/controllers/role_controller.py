...
from data.roles.models import RoleCategoryModel, RoleModel
...
async def request_role():
    ...
    bot.send_message(
        f'{user.username} a fait la demande pour le role {role_request.name} pour la categorie {role_request.role_category.name}',
